...
#define DEFAULT_MAX_BACKLOG				2048
 ...
 * producer reads a buffer, it is added to its queue, with a â€œseen
 * countâ€?of zero.
 *
 * Each consumer gets, at any time, the pointer to the start of the
 * queue and as it moves on to the following element, the â€œseen countâ€?
...
typedef struct {
     ...
     *
     * This value is the â€œserial numberâ€?of the buffer, which, simply
...
struct BufferQueue_Producer {
    /**
     * @brief reference count of producer.
     *
     * The consumer doesn't own the ref-count of producer, because procuder
     * has other way to keep track of consumers. 
     */
	gint ref_count;
    ...
    /**
     * @brief fd array for consumer waking up
     *
     * @todo any more
     */
	FD_set *fd_set;
    ...
    /**
     * @brief total packets count that input
     *
     * @unlike next_serial, total_packets never be reset
     */
	gulong	total_packets;


    /**
     * @brief minimal serial packet number in queue now.
     *
     * @'next_serial - min_serial' means packets count in queue.
     */
	gulong min_serial;
     ...
     * @brief max backlog
     *
     * @ bq_producer_put() will return error while backlog overflow.
     */
	gulong max_backlog;
...
struct BufferQueue_Consumer {
     ...
     *
     * This value is the â€œserial numberâ€?of the producer's queue,
...
gint bq_producer_test_ref_and_remove(BufferQueue_Producer *producer)
{
	...
	ref_count = g_atomic_int_get(&producer->ref_count);
	if (ref_count == 1) {
		if(producer->key) {
	        g_mutex_lock(bq_shared_producers_lock);
...
glong bq_producer_get_backlog(BufferQueue_Producer *producer)
{
	...
	g_mutex_lock(producer->lock);
	backlog = producer->next_serial - producer->min_serial;
	g_mutex_unlock(producer->lock);
...
gboolean bq_producer_unref(BufferQueue_Producer *producer)
{
	...
	if (!g_atomic_int_dec_and_test(&producer->ref_count))
		return FALSE;
...
gint bq_producer_put(BufferQueue_Producer *producer, gpointer payload) {
	...
	if (producer->next_serial - producer->min_serial >=
		producer->max_backlog)
	{
		g_mutex_unlock(producer->lock);
		bq_producer_reset_queue(producer);
		return -E_FULL;
	}
    ...
    ++producer->total_packets;

    /* Leave the exclusive access */
    g_mutex_unlock(producer->lock);

	fd_set_call(producer->fd_set);
...
gint bq_test_blockable(BufferQueue_Producer *producer, gint packets)
{
   if (g_atomic_int_get(&producer->stopped))
	...
	if (producer->next_serial - producer->min_serial + packets >=
		producer->max_backlog / 2)
	{
		g_mutex_unlock(producer->lock);
		return -E_FULL;
	}
...
void bq_producer_add_fd(BufferQueue_Producer *producer, gint fd,
	gpointer user_data, fd_callback_t call, fd_callback_t fin)
{
	g_assert(producer != NULL);

	fd_set_insert(producer->fd_set, fd, user_data, call, fin);
}
...
void bq_producer_remove_fd(BufferQueue_Producer *producer, gint fd)
{
	...
	fd_set_remove(producer->fd_set, fd);
}
...
static void bq_consumer_elem_unref(BufferQueue_Consumer *consumer) {
	...
	producer->min_serial = elem->serial;
...
gint bq_all_seen_element(gconstpointer a, gconstpointer b)
{
	BufferQueue_Element *elem = (BufferQueue_Element*)a;
	BufferQueue_Producer *producer = (BufferQueue_Producer*)b;
...
/**
 * @brief remove all elements seen by all consumers, when destroy a consumer
 *
 * @param producer
 *
 * @note This function will be called with lock held.
 */
void bq_remove_all_seen_elements(BufferQueue_Producer *producer)
{
	BufferQueue_Element *elem;
	GList *list;
	...
	while (TRUE) {
		list = g_queue_find_custom(producer->queue,
				 producer, bq_all_seen_element);
		if (!list)
			break;

		elem = list->data;
		bq_element_free_internal(elem, producer->free_function);
		g_queue_delete_link(producer->queue, list);
	}
}


/**
 * @brief Kill a consumer
 *
 * @param consumer The consumer object to kill
 *
 * @note This function will require exclusive access to the producer,
 *       and will thus lock its mutex.
 */
void bq_consumer_kill(BufferQueue_Consumer *consumer) {
   ...
   /* Compatibility with free(3) */
    ...
    if ( --producer->consumers == 0 ) {
		...
		bq_remove_all_seen_elements(producer);
    }
...
static gboolean bq_consumer_move_internal(BufferQueue_Consumer *consumer) {
    ...
    if ( (producer->queue_serial != consumer->queue_serial) ||
        ...
        consumer->current_element_pointer = NULL;	/* foreach reset */
        consumer->current_element_object = NULL;
...
gpointer bq_consumer_get(BufferQueue_Consumer *consumer) {
	...
	if ( (producer->queue_serial != consumer->queue_serial) ||
		...
		ret = ((BufferQueue_Element *)expected_next->data)->payload;
