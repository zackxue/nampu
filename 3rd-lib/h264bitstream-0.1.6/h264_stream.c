 ...
 *
 * Modifiyed by Shiyong, Zhang for vs2008, 2011/10/11
 */
#ifdef WIN32
...
{
    h264_stream_t* h = (h264_stream_t*)calloc(sizeof(h264_stream_t), 1);
...
void h264_free(h264_stream_t* h)
{
    ...
    for (index = 0; index < (sizeof(h->sps->ScalingList4x4) / sizeof(h->sps->ScalingList4x4[0])); index++)
    {
        if (h->sps->ScalingList4x4[index] != NULL)
            free(h->sps->ScalingList4x4[index]);
    }

    for (index = 0; index < (sizeof(h->sps->ScalingList8x8) / sizeof(h->sps->ScalingList8x8[0])); index++)
    {
        if (h->sps->ScalingList8x8[index] != NULL)
            free(h->sps->ScalingList8x8[index]);
    }
    ...
    if (h->seis)
    {
      for (index = 0; index < h->num_seis; index++)
      {
          free(h->seis[index]);
      }
    ...
    for (index = 0; index < (sizeof(h->pps->ScalingList4x4) / sizeof(h->pps->ScalingList4x4[0])); index++)
    {
        if (h->pps->ScalingList4x4[index] != NULL)
            free(h->pps->ScalingList4x4[index]);
    }

    for (index = 0; index < (sizeof(h->pps->ScalingList8x8) / sizeof(h->pps->ScalingList8x8[0])); index++)
    {
        if (h->pps->ScalingList8x8[index] != NULL)
            free(h->pps->ScalingList8x8[index]);
    }
        ...
        if( sps->seq_scaling_matrix_present_flag )
        {
            for( i = 0; i < 8; i++ )
            {
                ...
                if( sps->seq_scaling_list_present_flag[ i ] )
                {
                    if( i < 6 )
                    {
                        if (sps->ScalingList4x4[ i ] == NULL)
                            sps->ScalingList4x4[ i ] = (int *)malloc(16 * sizeof(int));
                    ...
                    {
                        if (sps->ScalingList8x8[ i - 6 ] == NULL)
                            sps->ScalingList8x8[ i - 6 ] = (int *)malloc(64 * sizeof(int));
        ...
        if( pps->pic_scaling_matrix_present_flag )
        {
            for( i = 0; i < 6 + 2* pps->transform_8x8_mode_flag; i++ )
            {
                ...
                if( pps->pic_scaling_list_present_flag[ i ] )
                {
                    if( i < 6 )
                    {
                        if (pps->ScalingList4x4[ i ] == NULL)
                            pps->ScalingList4x4[ i ] = (int *)malloc(16 * sizeof(int));
                    ...
                    {
                        if (pps->ScalingList8x8[ i - 6 ] == NULL)
                            pps->ScalingList8x8[ i - 6 ] = (int *)malloc(64 * sizeof(int));
...
int write_nal_unit(h264_stream_t* h, uint8_t* buf, int size)
{
    ...
    rbsp_buf = (uint8_t*)malloc(size*3/4); // NOTE this may have to be slightly smaller (3/4 smaller, worst case) in order to be guaranteed to fit
    rbsp_size = 0;
    nal_size = size;
