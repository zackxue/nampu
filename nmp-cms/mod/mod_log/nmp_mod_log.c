...
static JpfModLog *g_jpf_mod_log;

LOG_MSG_ENABLE g_log_msg_enable[LOG_MAX_MESSAGE_NUM];

G_DEFINE_TYPE(JpfModLog, jpf_mod_log, JPF_TYPE_APPMOD);
...
gint jpf_log_check_hook(JpfSysMsg *msg)
{
	JpfMsgID msg_id = MSG_GETID(msg);
	guint flag;
...
gint
jpf_mod_log_setup(JpfAppMod *am_self)
{
	JpfModLog *self;
	G_ASSERT(am_self != NULL);

	self = (JpfModLog*)am_self;

	jpf_app_mod_set_name(am_self, "MOD-LOG");
	jpf_mod_log_register_msg_handler(self);
...
static gboolean
jpf_mod_log_del_log_timer(gpointer user_data)
{
	JpfModLog * self = (JpfModLog *)user_data;
...
static void
jpf_mod_log_init(JpfModLog *self)
{
	g_jpf_mod_log = self;
	...
	jpf_set_timer(CHECK_LOG_TIME, jpf_mod_log_del_log_timer, self);
}


static void
jpf_mod_log_class_init(JpfModLogClass *k_class)
{
	JpfAppModClass *am_class = (JpfAppModClass*)k_class;

	am_class->setup_mod	= jpf_mod_log_setup;
}
