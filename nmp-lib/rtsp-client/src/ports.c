...
jpf_ports_get_pair(JpfPortsCtl *pc, gint *p_low, gint *p_hi)
{
	...
	__require_lock(pc);
	rc = __jpf_ports_get_pair(&jpf_media_ports, p_low, p_hi);
	__release_lock(pc);
...
jpf_ports_put_pair(JpfPortsCtl *pc, gint low, gint hi)
{
	...
	__require_lock(pc);
	__jpf_ports_put_pair(&jpf_media_ports, low, hi);
	__release_lock(pc);
}
...
{
	return jpf_ports_set_range(&jpf_media_ports, low, hi);
}
...
{
	jpf_ports_set_reserved(&jpf_media_ports, port);
}

__export gint
jpf_media_ports_get_one(gint *p_port)
{
	return jpf_ports_get_one(&jpf_media_ports, p_port);
}
...
{
	jpf_ports_put_one(&jpf_media_ports, port);
}
...
{
	return jpf_ports_get_pair(&jpf_media_ports, p_low, p_hi);
}
...
{
	jpf_ports_put_pair(&jpf_media_ports, low, hi);
}
...
{
	return jpf_ports_get_range(&jpf_media_ports, p_low, p_hi);
}
