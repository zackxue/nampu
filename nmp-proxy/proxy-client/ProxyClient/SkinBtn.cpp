...
CSkinBtn::CSkinBtn()
{
	...
	m_fg = RGB(255, 255, 255);
	m_bg = RGB(255, 255, 255);
	m_hover_bg = RGB(255, 0, 255);
	m_bMouseOver = false;
	m_bEnabled = true;
	m_bDCStored = false;
	...
	//			memDC.TextOut(m_textPos.x,m_textPos.y,str);
			...
			case HOVER:
				...
				memDC.DrawState(rImage.TopLeft(),
								...
								DSS_NORMAL, 
								(CBrush*)NULL);	
				memDC.SetTextColor(RGB(240,248,255));
	//			memDC.TextOut(m_textPos.x,m_textPos.y,str);
				if(m_bDrawRantage)
				{
					memDC.MoveTo(rect.left,rect.top);
...
{
	if (m_imgNormal.m_hObject)
		m_imgNormal.Detach();
	
	if (m_imgHover.m_hObject)
		m_imgHover.Detach();
	
	if (m_imgDown.m_hObject)
		m_imgDown.Detach();
	
	if (m_imgDisable.m_hObject)
		m_imgDisable.Detach();
...
//		//HFONT hFont = (HFONT)::SelectObject(lpDIS->hDC,m_font);
//
//
//		CRect rc;
//		GetWindowRect(&rc);
//		::DrawText(dc.m_hDC,strTitle,-1,&rc,DT_SINGLELINE);
//
//		//::SelectObject(lpDrawItemStruct->hDC,hFont);
//	}
//}

void CSkinBtn::SetTextColor(COLORREF clr)
{
	m_fg = clr;
	m_hover_bg = clr;
	//m_fg = RGB(255,255,255);
