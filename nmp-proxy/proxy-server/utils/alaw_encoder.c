//#ifndef __ALAW_ENCODER_H__
//#define __ALAW_ENCODER_H__
...
// alaw --> pcm
int g711a_Decode2(unsigned char *src, char *dest, int srclen, int *dstlen)
{
    int i;
    unsigned short *pd=(unsigned short*)dest;
    for(i=0; i<srclen; i++)
    {
        pd[i] = Snack_Alaw2Lin(src[i]);
        //pd[i]=(unsigned short)_A2l[src[i]];
    }
    *dstlen = srclen<<1;
