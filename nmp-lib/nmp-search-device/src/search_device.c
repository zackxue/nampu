...
static void show_device_list(search_array_t *array)
{
    ...
    for (i=0; i<array->count; i++)
    {
        printf("dst_id: %s, %s\n", array->result[i].dst_id, 
            array->result[i].jpf_srch.dev_info.dev_ip);
    }
    printf("\n");
}
#endif
...
static int search_notify(mb_cu_notify_t *notify_handle, mb_cu_parm_t *parm)
{
...
#if _DEBUG_
    jpf_search_t *jpf_srch = (jpf_search_t*)parm->data;
    device_info_t *dev_info = &jpf_srch->dev_info;
    ...
    memcpy(&result->jpf_srch, parm->data, sizeof(jpf_search_t));

    sscanf(result->jpf_srch.dev_info.dev_ip, "%d.%d.%d.%d", 
...
search_array_t *search_device()
{
    ...
    query_handle = mb_query(J_MB_Jpf_Search_Id, &notify_handle);
...
int set_platform_info(char *dst_id, jpf_redirect_t *jpf_red)
{
    ...
    mb_cfg_set(J_MB_Jpf_Redirect_Id, dst_id, 
        jpf_red->user_info.usr, 
        jpf_red->user_info.pwd, 
        timeout, 
        sizeof(redirect_t), 
        &jpf_red->redirect, 
