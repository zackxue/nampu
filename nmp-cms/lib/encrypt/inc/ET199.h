...
typedef struct _RTC_TIME_T {
	...
	BYTE week; /* day of the week, Sunday is 0, Monday is 1*/
...
//定时器 以下三个函数只在et199里能使用
extern BYTE _start_timer();
extern BYTE _stop_timer();
extern BYTE _get_timer(DWORD *result);

//以下三个函数只在ET金刚锁里能使用
extern BYTE _time(time_t *ptime);
//获取当前时钟时间，得到是一个time_t的变量

extern BYTE _mktime( time_t *ptime, RTC_TIME_T *ptm);
//把使用RTC_TIME_T结构体表示的时间转换成time_t表示的时间

extern BYTE _gmtime( time_t *ptime, RTC_TIME_T *ptm);
//把使用time_t数值表示的时间转换成RTC_TIME_T表示的时间
